...
export default function LoginAcerto() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!passOK) {
      setError("A senha precisa ter 8+ caracteres, 1 letra maiúscula e 1 número.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#0f2a24] text-white flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <main className="relative grid w-full max-w-6xl gap-8 md:grid-cols-2 items-center">
        <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="px-2">
          <div className="mb-6">
          ...
          <div className="text-center">
            <h1 className="text-4xl sm:text-5xl font-extrabold leading-tight tracking-tight">
              <motion.span
                ...
                whileHover={{ color: '#6ee7b7', textShadow: '0 2px 16px rgba(110,231,183,0.65)', scale: 1.05 }}
          ...
          <ul className="mt-6 space-y-3 text-emerald-100/90 pl-2">
            {['Crie grupos e registre despesas em segundos.', 'Veja quem deve para quem com cálculo otimizado.', 'Integração PIX para pagamentos instantâneos.'].map((txt, i) => (
          ...
          <div className="mt-8 flex items-center gap-4 text-sm text-emerald-200/80">
            <div className="flex items-center gap-2 hover:text-emerald-100 transition"><Smartphone className="h-4 w-4" /> Responsivo</div>
            <div className="flex items-center gap-2 hover:text-emerald-100 transition"><Mail className="h-4 w-4" /> Suporte rápido</div>
          </div>
        </motion.section>

        <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.05 }} className="px-2">
          <motion.div whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 250, damping: 22 }}>
            <Card className="bg-emerald-900/50 border-emerald-800/60 text-emerald-50 shadow-xl rounded-2xl">
              <CardContent className="p-6 sm:p-8">
                <div className="mb-6 text-center">
                  <p className="text-xs text-emerald-100/70 tracking-widest uppercase">Bem-vindo(a) ao</p>
                  <p className="text-3xl font-extrabold mt-1">
                    <motion.span className="inline-block text-emerald-300 cursor-pointer select-none" initial={{ opacity: 0, y: 6, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} whileHover={{ color: '#6ee7b7', textShadow: '0 2px 16px rgba(110,231,183,0.65)', scale: 1.05 }}>AcertÔ</motion.span>
                  </p>
                  <motion.p className="mt-2 text-sm text-[#6ee7b7]" style={{ textShadow: '0 0 10px rgba(110,231,183,0.55)' }} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.12 }}>A conta vai, a amizade fica.</motion.p>
                </div>

                {/* Login oficial com NextAuth */}
                <div className="space-y-3 mb-5">
                  <Button type="button" onClick={() => signIn('google')} className="w-full flex items-center justify-center gap-3 rounded-xl bg-white text-gray-700 border border-gray-300 h-11 hover:shadow-lg hover:-translate-y-[1px] transition font-medium">
                    <img src="https://developers.google.com/identity/images/g-logo.png" alt="Google logo" className="h-5 w-5" /> Entrar com Google
                  </Button>
                  <Button type="button" onClick={() => signIn('twitter')} className="w-full flex items-center justify-center gap-3 rounded-xl bg-black text-white border border-gray-700 h-11 hover:shadow-lg hover:-translate-y-[1px] transition font-medium">
                    <img src="https://abs.twimg.com/icons/apple-touch-icon-192x192.png" alt="X logo" className="h-5 w-5" /> Entrar com X
                  </Button>
                </div>

                <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                  <div className="space-y-2">
                    <Label htmlFor="email" className="text-emerald-100">E‑mail</Label>
                    <Input id="email" type="email" placeholder="voce@email.com" className="bg-emerald-950/40 border-emerald-800/70 focus-visible:ring-emerald-400 hover:border-emerald-400/70 transition" value={email} onChange={(e) => setEmail(e.target.value)} required />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="password" className="text-emerald-100">Senha</Label>
                    <div className="relative">
                      <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" className="bg-emerald-950/40 border-emerald-800/70 pr-10 focus-visible:ring-emerald-400 hover:border-emerald-400/70 transition" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} required />
                  ...
                  <Button type="submit" disabled={loading} className="w-full h-11 rounded-xl bg-emerald-500 hover:bg-emerald-400 hover:shadow-lg hover:-translate-y-[1px] text-emerald-950 font-semibold transition">
                    <LogIn className="h-5 w-5 mr-2" /> {loading ? "Entrando…" : "Entrar"}
                  </Button>
